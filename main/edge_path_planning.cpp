...
std::vector<std::vector<cv::Point>> edge_path_coordinates(cv::Mat &image, const cv::Point offset) {
    ...
    while (streak.size()) {
        ...
        } streak = generate_edge_streak(image, offset);
    }
    ...
    while (queue.size()) {
        ...
        if (image.at<uchar>(current) == 255) {
            ...
            if (neighbours.size()>1) {
                ...
                std::sort(neighbours.begin(), neighbours.end(), [&](const cv::Point &p1, const cv::Point &p2){return get_neighbours(p1, image).size()<get_neighbours(p2, image).size();}); // Selector `&` used in place of `&image`
